...
// Check if ViewContent has been fired THIS SESSION (sessionStorage — fires once per new visit)
const hasViewContentFired = () => {
  if (typeof window === "undefined") return false;
  try {
    return sessionStorage.getItem(PAGEVIEW_FIRED_KEY) === "true";
  } catch (err) {
    return false;
  }
};

// Mark ViewContent as fired for this session
const markViewContentFired = () => {
  if (typeof window === "undefined") return;
  try {
    sessionStorage.setItem(PAGEVIEW_FIRED_KEY, "true");
  } catch (err) {
    console.warn("⚠️ Failed to persist ViewContent fired state", err);
  }
};

// Check if InitiateCheckout has been fired THIS SESSION (once per session)
const hasInitiateCheckoutFired = () => {
  if (typeof window === "undefined") return false;
  try {
    return sessionStorage.getItem(INITIATECHECKOUT_FIRED_KEY) === "true";
  } catch (err) {
    return false;
  }
};

// Mark InitiateCheckout as fired for this session
const markInitiateCheckoutFired = () => {
  if (typeof window === "undefined") return;
  try {
    sessionStorage.setItem(INITIATECHECKOUT_FIRED_KEY, "true");
  } catch (err) {
...
const trackMetaPixel = (eventName, eventData = {}) => {
  ...
  try {
    ...
    // Initialize if needed (idempotent in fbq)
    window.fbq("init", META_PIXEL_ID);
    window.fbq("track", eventName, eventData);
    console.log(`✅ Meta Pixel: ${eventName} tracked with ID ${META_PIXEL_ID}`, eventData);
  } catch (err) {
...
function App() {
  ...
  const checkoutEventFiredRef = useRef(hasInitiateCheckoutFired()); // Track if InitiateCheckout has been fired this session
  const pageViewFiredRef = useRef(hasViewContentFired()); // Track if ViewContent has been fired this session
  ...
  // ✅ [PIXEL 1] ViewContent — fires INSTANTLY on app mount, once per session
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (pageViewFiredRef.current) return; // already fired this session

    const fireViewContent = () => {
      if (typeof window.fbq !== "function") return;
      window.fbq("init", META_PIXEL_ID);
      window.fbq("track", "ViewContent", {
        content_name: "Instagram Stalker - Landing Page",
        content_category: "Stalker App",
        currency: "INR",
        value: 0,
      });
      pageViewFiredRef.current = true;
      markViewContentFired();
      console.log("✅ Meta Pixel: ViewContent fired (session first visit)");
    };

    // fbevents.js loads async — poll until ready (max ~3s)
    if (typeof window.fbq === "function") {
      ...
      const poll = setInterval(() => {
        ...
        if (typeof window.fbq === "function") {
          ...
          clearInterval(poll);
          console.warn("⚠️ Meta Pixel: fbq never loaded, ViewContent skipped");
        }
      }, 100);
    }
  }, []); // empty deps = mount only

  // ✅ [PIXEL 2] InitiateCheckout — fires INSTANTLY when payment screen shown, once per session
  useEffect(() => {
    if (typeof window === "undefined" || screen !== SCREEN.PAYMENT) return;
    if (checkoutEventFiredRef.current) return; // already fired this session

    let poll = null; // hold ref for cleanup

    const fireInitiateCheckout = () => {
      if (checkoutEventFiredRef.current) return; // inner guard — prevents race condition
      if (typeof window.fbq !== "function") return;
      checkoutEventFiredRef.current = true; // set BEFORE fbq call to block any concurrent poll
    ...
    // Fire immediately if fbq is ready, else poll until ready (max ~3s)
    if (typeof window.fbq === "function") {
      ...
      poll = setInterval(() => {
        ...
        if (typeof window.fbq === "function") {
          ...
          clearInterval(poll);
          poll = null;
          console.warn("⚠️ Meta Pixel: fbq never loaded, InitiateCheckout skipped");
        }
      }, 100);
    }

    // Cleanup: if screen changes before poll completes, kill the interval
    return () => {
      if (poll) {
        clearInterval(poll);
        poll = null;
  ...
  // ✅ [PIXEL 3] Purchase — fires INSTANTLY when success screen shown
  // Backend redirects to /post-purchase after Paytm — pending order in localStorage, fire immediately.
  useEffect(() => {
    ...
    if (typeof window.fbq === "function") {
      ...
      console.log("✅ Meta Pixel: Purchase pixel fired:", orderId);
          ...
          const messageTemplates = [
            ...
            () => {
              ...
              return `${item.username
                } mentioned you in ${messageCount} message${messageCount > 1 ? "s" : ""
                }`;
            },
            () => {
                ...
                <div className="visitor-stack">
                  {hero.visitors.map((visitor, index) => (
                    <div
                      ...
                      key={`visitor-${index}-${visitor.isLocked ? "locked" : "visible"
                        }`}
                    >
                      {visitor.isLocked ? (
                ...
                <div className="carousel-container">
                  <div className="carousel-wrapper">
                    <div
                      ...
                      style={{
                        transform: `translateX(calc(-${displayIndex * (220 + 16)
                          }px))`,
                      ...
                      {duplicatedCards.map(
                        ({ card, originalIndex, duplicateKey }, index) => {
                          ...
                          if (isMultipleOf5 && imageUrl) {
                            return (
                              <article
                                className={`slider-card slider-card--blurred ${isActive ? "active" : ""
                                  }`}
                          ...
                          if (isLocked) {
                            return (
                              <article
                                className={`slider-card slider-card--locked ${isActive ? "active" : ""
                                  }`}
                  ...
                  <div className="carousel-container">
                    <div className="carousel-wrapper">
                      <div
                        className="carousel-track"
                        style={{
                          transform: `translateX(calc(-${displayStoriesIndex * (220 + 16)
                            }px))`,
